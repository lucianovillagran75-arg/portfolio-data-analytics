...
Modelé los datos en un **esquema estrella** (una tabla de hechos + cuatro dimensiones) y escribí
...
```
fact_sales (15.700 líneas de venta)
│   fecha · cantidad · precio · descuento · costo · revenue · margin
│
├── dim_customers   → segmento · ciudad · región · alta
├── dim_products    → categoría · costo · precio de lista
└── dim_stores      → sucursal · ciudad · región
```
...
| 03 | [`03_churn_clientes.sql`](./queries/03_churn_clientes.sql) | ¿Cuántos clientes perdimos y cuánto valían? |
...
|---|---|---|---|
| Productos vendidos bajo margen | No medido | **4 SKU · $1,82 M acumulado** | **$0,91 M/año** al repreciar |
| Clientes en churn | Invisible | **156 clientes (21,7 %)** | **$0,74 M/año** con win-back |
| Brecha de sucursal | Invisible | **Sur: −16 % de ticket** | **$0,39 M/año** al cerrar la brecha |
| Capital en baja rotación | No medido | **5 SKU · $79 k** | Capital liberable |
...
WITH activos_2024 AS (...), activos_2025 AS (...), churn AS (...)

-- Agregación de negocio sobre la tabla de hechos
SUM(margin), SUM(revenue), AVG(discount)

-- Agrupación temporal sin columnas extra
strftime('%Y-%m', date)   ·   substr(date, 1, 7)

-- Comparación contra línea base (un número solo no dice nada)
ticket_promedio vs. mediana de la red
```

- Modelado en **esquema estrella** (hechos + dimensiones) con índices para performance.
...
*Datos simulados con distribuciones realistas del sector retail · Portfolio de Data & Analytics*
